...
app.get("/scrape", (req, res) => {
  ...
  let startDate =
    (!!startDateParam && DateTime.fromFormat(startDateParam, Y_M_D)) ||
    DateTime.now().minus({ months: 1 });
  let endDate =
    (!!endDateParam && DateTime.fromFormat(endDateParam, Y_M_D)) ||
    DateTime.now();
  ...
  runScraper(startDate, endDate)
    .then((r) => {
      if (!!r.error) {
        ...
        // res.send(r.results);
        let numFailures =
          !!r.failures && !!r.failures.length ? r.failures.length : 0;

        console.log(
          `Render Method: ${numFailures} processing failure${
            numFailures === 1 ? "" : "s"
          }:
